...
private final class PrivacyAndSecurityControllerArguments {
    ...
    init(account: Account, openBlockedUsers: @escaping () -> Void, openLastSeenPrivacy: @escaping () -> Void, openGroupsPrivacy: @escaping () -> Void, openVoiceCallPrivacy: @escaping () -> Void, openProfilePhotoPrivacy: @escaping () -> Void, openForwardPrivacy: @escaping () -> Void, openPhoneNumberPrivacy: @escaping () -> Void, openVoiceMessagePrivacy: @escaping () -> Void, openBioPrivacy: @escaping () -> Void, openBirthdayPrivacy: @escaping () -> Void, openSavedMusicPrivacy: @escaping () -> Void, openPasscode: @escaping () -> Void, openTwoStepVerification: @escaping (TwoStepVerificationAccessConfiguration?) -> Void, openPasskeys: @escaping () -> Void, openActiveSessions: @escaping () -> Void, toggleArchiveAndMuteNonContacts: @escaping (Bool) -> Void, setupAccountAutoremove: @escaping () -> Void, setupMessageAutoremove: @escaping () -> Void, openDataSettings: @escaping () -> Void, openBrowserSelection: @escaping () -> Void, openEmailSettings: @escaping (String?) -> Void, openMessagePrivacy: @escaping () -> Void, openGiftsPrivacy: @escaping () -> Void, updateGhostModeSettings: @escaping (@escaping (GhostModeSettings) -> GhostModeSettings) -> Void) {
        ...
        self.updateGhostModeSettings = updateGhostModeSettings
    }
}
...
private enum PrivacyAndSecurityEntry: ItemListNodeEntry {
    ...
    case ghostModeHeader(String)
    case ghostModeOnline(Bool)
    case ghostModeInputActivities(Bool)
    case ghostModeReadReceipts(Bool)
    case ghostModeInfo(String)
    ...
    var section: ItemListSectionId {
        ...
        case .ghostModeHeader, .ghostModeOnline, .ghostModeInputActivities, .ghostModeReadReceipts, .ghostModeInfo:
    ...
    var stableId: Int32 {
        switch self {
            ...
            case .ghostModeOnline:
                return 35
            case .ghostModeInputActivities:
                return 36
            case .ghostModeReadReceipts:
                return 37
            case .ghostModeInfo:
                return 38
        }
    ...
    static func ==(lhs: PrivacyAndSecurityEntry, rhs: PrivacyAndSecurityEntry) -> Bool {
        switch lhs {
            ...
            case let .ghostModeHeader(lhsText):
                if case let .ghostModeHeader(rhsText) = rhs {
                    return lhsText == rhsText
                } else {
                    return false
                }
            case let .ghostModeOnline(lhsValue):
                if case let .ghostModeOnline(rhsValue) = rhs {
                    return lhsValue == rhsValue
                } else {
                    return false
                }
            case let .ghostModeInputActivities(lhsValue):
                if case let .ghostModeInputActivities(rhsValue) = rhs {
                    return lhsValue == rhsValue
                } else {
                    return false
                }
            case let .ghostModeReadReceipts(lhsValue):
                if case let .ghostModeReadReceipts(rhsValue) = rhs {
                    return lhsValue == rhsValue
                } else {
                    return false
                }
            case let .ghostModeInfo(lhsText):
                if case let .ghostModeInfo(rhsText) = rhs {
                    return lhsText == rhsText
                } else {
                    return false
                }
    ...
    func item(presentationData: ItemListPresentationData, arguments: Any) -> ListViewItem {
        ...
        switch self {
            ...
            case let .ghostModeHeader(text):
                return ItemListSectionHeaderItem(presentationData: presentationData, text: text, sectionId: self.section)
            case let .ghostModeOnline(value):
                return ItemListSwitchItem(presentationData: presentationData, systemStyle: .glass, title: "Не показывать онлайн", value: value, sectionId: self.section, style: .blocks, updated: { value in
                    arguments.updateGhostModeSettings { settings in
                        var settings = settings
                        settings.hideOnlineStatus = value
                        return settings
                    }
                })
            case let .ghostModeInputActivities(value):
                return ItemListSwitchItem(presentationData: presentationData, systemStyle: .glass, title: "Скрывать набор текста", value: value, sectionId: self.section, style: .blocks, updated: { value in
                    arguments.updateGhostModeSettings { settings in
                        var settings = settings
                        settings.hideInputActivities = value
                        return settings
                    }
                })
            case let .ghostModeReadReceipts(value):
...
private func privacyAndSecurityControllerEntries(
    ...
    entries.append(.ghostModeHeader("РЕЖИМ ПРИЗРАКА"))
    entries.append(.ghostModeOnline(ghostModeSettings.hideOnlineStatus))
    entries.append(.ghostModeInputActivities(ghostModeSettings.hideInputActivities))
    entries.append(.ghostModeReadReceipts(ghostModeSettings.hideReadReceipts))
    entries.append(.ghostModeInfo("Время отправленного сообщения всё равно может косвенно показать активность. Прочтения синхронизируются после отключения режима."))
...
public func privacyAndSecurityController(
    ...
    }, updateGhostModeSettings: { f in
        GhostModeSettingsStore.shared.update(f)
    })
    ...
    let signal = combineLatest(
        ...
        loginEmail,
        GhostModeSettingsStore.shared.settings
    )
    |> map { presentationData, state, privacySettings, noticeView, webBrowserData, recentPeers, blockedPeersState, activeWebsitesState, accessChallengeData, twoStepAuth, passkeys, appConfiguration, accountPeer, loginEmail, ghostModeSettings -> (ItemListControllerState, (ItemListNodeState, Any)) in
