...
private final class AccountPresenceManagerImpl {
    ...
    init(queue: Queue, shouldKeepOnlinePresence: Signal<Bool, NoError>, ghostModeSettings: Signal<GhostModeSettings, NoError>, network: Network) {
        self.queue = queue
        self.network = network
        
        self.shouldKeepOnlinePresenceDisposable = (combineLatest(shouldKeepOnlinePresence, ghostModeSettings)
        |> map { shouldKeepOnlinePresence, settings in
            return shouldKeepOnlinePresence && !settings.hideOnlineStatus
        }
...
final class AccountPresenceManager {
    ...
    init(shouldKeepOnlinePresence: Signal<Bool, NoError>, ghostModeSettings: Signal<GhostModeSettings, NoError>, network: Network) {
        let queue = self.queue
        self.impl = QueueLocalObject(queue: self.queue, generate: {
            return AccountPresenceManagerImpl(queue: queue, shouldKeepOnlinePresence: shouldKeepOnlinePresence, ghostModeSettings: ghostModeSettings, network: network)
        })
    }
