...
private final class SynchronizePeerReadStatesContextImpl {
    ...
    init(queue: Queue, network: Network, postbox: Postbox, stateManager: AccountStateManager) {
        ...
        self.ghostModeSettings = GhostModeSettingsStore.shared.current
        ...
        self.ghostModeDisposable = (GhostModeSettingsStore.shared.settings
        |> deliverOn(self.queue)).start(next: { [weak self] settings in
            guard let self else {
                return
            }
            self.ghostModeSettings = settings
        ...
        for peerId in peerIds {
            ...
            if let operation = maybeOperation {
                if let current = self.activeOperations[peerId] {
                    ...
                    if self.ghostModeSettings.hideReadReceipts {
                        continue
                    }
