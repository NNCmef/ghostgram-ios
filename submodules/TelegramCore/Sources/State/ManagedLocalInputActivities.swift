...
func managedLocalTypingActivities(activities: Signal<[PeerActivitySpace: [(PeerId, PeerInputActivityRecord)]], NoError>, ghostModeSettings: Signal<GhostModeSettings, NoError>, postbox: Postbox, network: Network, accountPeerId: PeerId) -> Signal<Void, NoError> {
    return Signal { subscriber in
        let context = Atomic(value: ManagedLocalTypingActivitiesContext())
        let disposable = (combineLatest(activities, ghostModeSettings)
        |> map { activities, settings in
            return settings.hideInputActivities ? [:] : activities
        }).start(next: { activities in
