...
The GitHub Actions workflow builds an IPA on a macOS runner. Add these repository secrets before running it:

- `TG_API_ID` and `TG_API_HASH`: credentials created at https://my.telegram.org/apps
- `TG_BUNDLE_ID`: a unique bundle identifier that does not use Telegram branding
- `TG_TEAM_ID`: the Apple Developer team identifier

The produced IPA uses development-only fake signing and must be re-signed by the installation method before it can run on a physical device.
